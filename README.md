...
В папке `backend/app/templates` должны лежать шаблоны для процедур из выпадающего списка формы:
...
Для каждого выбранного пункта генерируется один комплект DOCX/PDF по соответствующему шаблону.
...
2. Нажмите «НАЧАТЬ», в модальном окне на шаге 1 выберите тип согласия (на себя / на ребенка / на лицо, чьим представителем вы являетесь) и заполните базовые данные.
3. На шаге 2 выберите услугу.
4. На шаге 3 заполните динамические поля (аллергия, поля по имплантации или седации при необходимости), поставьте подпись и отметьте согласие на обработку ПД.
